let
    ...
    */
    // 1
    group_undidade_negocio = Table.Group(add_produtos_em_linha,"Família",{
        {"group_familia", each _, type table}
    }),
    // 2
    group_por_mes_pareto = Table.TransformColumns(group_undidade_negocio,{"group_familia", (tb_familia) => (
        Table.Group(tb_familia,"mes_num",{"group_mes_num", each _, type table})
    ), type table}),
    // 3 4 5 6
    index_mes_pareto = Table.TransformColumns(group_por_mes_pareto, {"group_familia",(tb_familia) => (
        Table.TransformColumns(tb_familia,{"group_mes_num", (tb_mes_num) => (
            let
                // cria o indice dos defeitos em ordem
                tb_index_mes = Table.AddIndexColumn((
                    Table.Sort((
                        Table.AddColumn(
                            Table.Group(tb_mes_num,"Defeito Constatado",{"quant", each List.Sum([quant]), type number})
                ...
                // Nested Join no pareto e no indice de volta à tabela tb_mes_num
                traz_index_mes_pareto = Table.ExpandTableColumn(
                    Table.NestedJoin(tb_mes_num,"Defeito Constatado",pareto,"Defeito Constatado","dados",JoinKind.LeftOuter)
    ...
    // 7 abrir os groups
    abrir_os_groups = Table.ExpandTableColumn((
        Table.ExpandTableColumn(index_mes_pareto,"group_familia",{
        "mes_num","group_mes_num"
        
    })), "group_mes_num",{
        "ano_num","quartil_num","Referência Produto","Defeito Constatado",
        "quant","filtro_mes","filtro_quartil","ano_texto","mes_texto",
        "quartil_texto","relação","relacao_defeito","em_linha","pareto","index_mes_pareto"
    }),
